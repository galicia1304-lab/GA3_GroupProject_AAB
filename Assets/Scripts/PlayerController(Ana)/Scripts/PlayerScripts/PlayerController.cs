...
public class PlayerController : MonoBehaviour
{
	...
	void HandleFootsteps()
	{
		...
		// Different step distances
		float stepDistance =
			speed > runSpeed * 0.5f ? 1.1f :   // running
			speed > walkSpeed * 0.3f ? 1.6f :  // walking
			1.2f;                              // sneaking

		if (distanceTraveled < stepDistance) return;
		...
		if (Physics.Raycast(rayOrigin, Vector3.down, out hit, rayLength))
		{
			...
			if (layer == LayerMask.NameToLayer("Grass"))
				footstepSource.PlayOneShot(grassFootstep);
			else if (layer == LayerMask.NameToLayer("Stone"))
				footstepSource.PlayOneShot(stoneFootstep);
